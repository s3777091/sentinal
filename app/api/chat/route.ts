    ...
    if (user_detail) {
      if (user_detail.apiServerId) {
        ...
        if (userCountOnServer > 5) {
          ...
          const leastPopulatedServer = await prisma.apiServer.findFirst({
            orderBy: {
              users: {
                _count: "asc",
              },
          ...
          apiServerToUse = leastPopulatedServer?.apiUrl || null;

          // Update user's apiServerId to the new server's ID
          if (leastPopulatedServer) {
            await prisma.user.update({
              where: { id: user_detail.id },
              data: { apiServerId: leastPopulatedServer.id },
            });
          }
        } else {
        ...
        // If user is not assigned to any server, assign to the server with the least users
        const leastPopulatedServer = await prisma.apiServer.findFirst({
          orderBy: {
            users: {
              _count: "asc",
